...
const BoardingPass = () => {
  ...
  const appendLastPartOfUrl = (url) => {
    const baseUrl = "https://scannables.scdn.co/uri/plain/jpeg/000000/white/640/spotify:user:";
    const lastPart = url.split('/').pop();
    return `${baseUrl}${lastPart}`;
  };
  ...
  const handleDownload = async () => {
    ...
    // Force reflow to ensure all elements are properly rendered
    const boardingPass = componentRef.current;
    boardingPass.style.display = 'none';
    const reflow = boardingPass.offsetHeight; // Trigger reflow
